...
- **Currently working on:** Phase 0 & Phase 1 are 100% complete end-to-end and deployed live. Full UI/UX Pro Max overhaul across Web & Mobile complete according to `design.md` and `ui-ux-pro-max` skillset (8s request timeout in `@elearny/api-client`, 1-Click Instant Demo Login for Student/Instructor/Admin to eliminate cloud cold-start delay during testing, fixed Expo Metro Feather icon warnings, updated high-contrast modern Web & Mobile layouts). Next: Starting Phase 2 Unit 1 (`InstructorApplication` entity & Flyway migration `V3__instructor_applications.sql`).
